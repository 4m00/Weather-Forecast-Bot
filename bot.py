...
open_weather_token = "YOUR_OPEN_WEATHER_API_TOKEN"
tg_bot_token = "YOUR_TELEGRAM_BOT_TOKEN"

async def get_weather(message: types.Message):
    code_to_smile = {"Clear": "Ясно \U00002600", "Clouds": "Облачно \U00002601", "Rain": "Дождь \U00002614",
                     "Drizzle": "Дождь \U00002614", "Thunderstorm": "Гроза \U000026A1", "Snow": "Снег \U0001F328",
    ...
    try:
        ...
        wd = code_to_smile[weather_description] if weather_description in code_to_smile else "Посмотри в окно, не пойму что там за погода!"
        ...
        await message.reply(f"***{datetime.datetime.now().strftime('%d-%m-%Y %H:%M')}***\n"
              ...
              f"Хорошего дня!")
        dat = [temp_min, cur_weather, temp_max]
        res = stats.ttest_1samp(a=dat, popmean=0)
        alpha = 0.05
        if res.pvalue < alpha:
            print("Отвергаем нулевую гипотезу")
        else:
            print("Не получилось отвергнуть нулевую гипотезу")
        marks = pd.Series({"Мин. темп": temp_min, "Темп": cur_weather, "Макс. темп": temp_max})
